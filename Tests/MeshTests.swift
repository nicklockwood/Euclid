...
final class MeshTests: XCTestCase {
    ...
    func testInsetHandBuiltBoxMatchesExpectedBounds() {
        let material = "box"
        let mesh = Mesh(closedBoxPolygons(size: [2, 3, 4], material: material)).inset(by: 0.25)
...
private extension MeshTests {
    func closedBoxPolygons(size: Vector, material: Mesh.Material? = nil) -> [Euclid.Polygon] {
        let x = size.x / 2
        let y = size.y / 2
        let z = size.z / 2
        return [
            Polygon([[-x, -y, -z], [-x, y, -z], [x, y, -z], [x, -y, -z]], material: material)!,
            Polygon([[-x, -y, z], [x, -y, z], [x, y, z], [-x, y, z]], material: material)!,
            Polygon([[-x, -y, -z], [x, -y, -z], [x, -y, z], [-x, -y, z]], material: material)!,
            Polygon([[x, -y, -z], [x, y, -z], [x, y, z], [x, -y, z]], material: material)!,
            Polygon([[x, y, -z], [-x, y, -z], [-x, y, z], [x, y, z]], material: material)!,
