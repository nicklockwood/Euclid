...
public extension Path {
    ...
    static func union<T: Collection>(_ paths: T) -> [Path] where T.Element == Path {
        ...
        case 1:
            ...
            return subpaths.count == 1 ? subpaths : symmetricDifference(subpaths)
        default:
            var result = symmetricDifference(paths.first!.subpaths)
            for path in paths.dropFirst() {
                result = result.union(symmetricDifference(path.subpaths))
            }
            return result
        }
    }
    ...
    static func difference<T: Collection>(_ paths: T) -> [Path] where T.Element == Path {
        ...
        default:
            var result = symmetricDifference(paths.first!.subpaths)
            for path in paths.dropFirst() {
                result = result.clip(to: symmetricDifference(path.subpaths))
            }
            return result
        }
    }
    ...
    func symmetricDifference(_ path: Path) -> [Path] {
        Self.symmetricDifference([self, path])
    }
    ...
    /// - Returns: An array of paths representing the XOR of the input paths.
    static func symmetricDifference<T: Collection>(_ paths: T) -> [Path] where T.Element == Path {
        switch paths.count {
        case 0:
            return []
        case 1:
        ...
        default:
            var result = symmetricDifference(paths.first!.subpaths)
            for path in paths.dropFirst() {
                result = result.symmetricDifference(symmetricDifference(path.subpaths))
            }
            return result
        }
    }
...
private extension Array where Element == Path {
    func symmetricDifference(_ paths: [Path]) -> [Path] {
